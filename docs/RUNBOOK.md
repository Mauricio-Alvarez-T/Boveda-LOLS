...
### Runtime
- **Node.js + Phusion Passenger** — startup file: **`index.js`** en AMBOS entornos (línea
  `PassengerStartupFile` del `api/.htaccess` del docroot). `app.js` queda **DEPRECADO** como
  startup: su try/catch traga el error de `require('./index')` sin re-lanzar ni exportar →
  Passenger recibe un módulo vacío y muestra "Web application could not be started" sin causa
  visible. `server.js` NUNCA existió en el repo (ver §6, caída 2026-08-24).
- **`node_modules` en el servidor es un SYMLINK** → `~/nodevenv/<app>/20/lib/node_modules`
  (CloudLinux NodeJS Selector). Recrear el venv (cambiar versión de Node + SAVE en el panel)
  **ROMPE el symlink** aunque las libs sobrevivan intactas. Lo reparan: **Run NPM Install**
  (Setup Node.js App) o el bloque `heal_passenger` de `scripts/cpanel-deploy-*.sh` en el
  próximo tick del cron.
- **Reinicio de app:** escribir cualquier cosa en `tmp/restart.txt` dentro del directorio del
  backend (`~/boveda` o `~/test-boveda`). En ambos entornos lo hace el script pull-side del cron;
  a mano se puede desde File Manager (no depende del panel ni de FTP).
- **⚠️ SAVE en Setup Node.js App es una operación de riesgo en esta cuenta** (caída 2026-08-24:
  un SAVE rompió el mapeo CageFS del intérprete de AMBOS venv y el panel quedó rechazando toda
  operación). Para cambios menores, editar la línea correspondiente del `.htaccess` a mano.
- **Topología de PROD (legado):** su Application URL quedó mal armado (dominio `lols.cl` + ruta
  `boveda.lols.cl`), por lo que el bloque Passenger vive en el **`.htaccess` del DOCROOT**
  (`~/public_html/boveda.lols.cl/.htaccess`, con `PassengerNodejs` duplicado) y NO en
  `api/.htaccess` como staging. Su startup actual es `app.js` hasta normalizar a `index.js`.
...
3. **Crear App Node.js** en cPanel → Setup Node.js App → Create Application:
   ...
   - Application startup file: `index.js` — ⚠️ NUNCA `server.js`: ese archivo NO existe en el repo.
     El valor que muestra el formulario de cPanel se REESCRIBE al `.htaccess` de Passenger cada vez
     que se aprieta SAVE; guardar con `server.js` tumba la app ("Web application could not be
...
| **Producción** (`main`) | **Pull-side cron-only ✅ LIVE (2026-06-16)**: Actions compila y publica la rama `deploy-prod`; un **cron self-healing en cPanel** (cada 5 min) auto-despliega con `scripts/cpanel-deploy-prod.sh` (ver PLAYBOOK §7) | El mismo baneo cPHulk afecta al FTP de prod. El FTP quedó OBSOLETO para deploy. |
...
**En staging y producción: AUTOMÁTICO desde 2026-08-27 (bloque `auto-migrate`).**

El cron de deploy (`scripts/cpanel-deploy-staging.sh` / `-prod.sh`) corre `node scripts/migrate.js`
él mismo cada vez que el deploy trae archivos nuevos en `db/migrations/`. No hay que hacer nada:
push → Actions publica la rama `deploy-*` → el cron despliega y migra en el mismo tick (o el
siguiente, ≤5 min, porque el cron pre-carga el `.sh` y el script nuevo rige desde el tick posterior
al deploy que lo trae).

- **Cómo verificar:** `https://test.boveda.lols.cl/migrate-status.txt` y
  `https://boveda.lols.cl/migrate-status.txt` — una línea con fecha + `OK` (y el conteo de
  migraciones registradas) o `FALLO rc=N`. El log completo queda en `tmp/migrate-auto.log` del
  backend (File Manager).
- **Mecánica:** la "firma" es el `md5sum` ordenado de los `*.sql` desplegados (por CONTENIDO: un
  `.sql` corregido in situ dispara de inmediato y resetea el backoff); se compara contra
  `tmp/.migrate-aplicado.lista` (escrito solo al ÉXITO — y éxito exige rc 0 **y** salida no vacía,
  para que un `bin/node` vacío degradado a `sh` no produzca un OK falso). Fallo → reintento cada
  30 min del mismo set (backoff vía `tmp/.migrate-intento.lista` + mtime del log); un set nuevo
  migra de inmediato. Lock anti-solape en `tmp/.migrate-auto.lock`; si `heal-deps` está instalando
  (su lock presente), pospone. El deploy además **espeja** `db/migrations/` con `--delete` (paso
  3a) para que un `.sql` borrado/renombrado en el repo no quede huérfano reintentando para siempre.
  Al aplicar toca `tmp/restart.txt` (el backend cachea columnas por proceso — `existingCols`).
- **Por qué existe:** el camino del panel (abajo) está **roto** desde el restore del 2026-08-24 —
  el Selector de cPanel lanza los scripts con `~/nodevenv/<app>/20/bin/npm`, binario que el restore
  no devolvió → `FileNotFoundError` antes de ejecutar nada. Es la generalización del `migrate-once`
  del incidente (commit `aee3aef`).

**Camino manual del panel (⚠️ ROTO desde 2026-08-24, se deja por referencia histórica):**
...
| Fechas del reporte/emails salen **en inglés** ("Mon Jun 22") | mysql2 devuelve columnas `DATE` como **objeto `Date`** (el typeCast de `db.js` solo castea `TINY(1)`→boolean, no fechas). `String(dateObj).slice(0,10)` = "Mon Jun 22", y `fmtFecha` lo pasa tal cual porque no matchea `YYYY-MM-DD`. | Normalizar con un helper `toYmd(v)` (`v instanceof Date ? ymd(v) : extraer /^\d{4}-\d{2}-\d{2}/`) ANTES de `fmtFecha`. Aplica a toda columna DATE cruda que se renderice. |
...
| **"Web application could not be started"** en `/api/*` justo después de GUARDAR la app en Setup Node.js App (frontend estático sigue 200); el panel responde **`No such application (or application not configured)`** a cualquier SAVE posterior | **Un SAVE en Setup Node.js App reescribe el registro del selector** (`~/.cl.selector/node-selector.json`) **y puede ROMPER el mapeo CageFS del intérprete**: `~/nodevenv/<app>/20/bin/` queda VACÍO (cambia el ctime, el mtime queda intacto — no se borró nada; se perdió un vínculo que solo root recrea). Sin binario, el panel valida la app ANTES de operar y rechaza incluso la operación que reconstruiría el venv → **bucle cerrado a nivel usuario** (`cloudlinux-selector` no existe en el cron, no hay Terminal). Verificado en la caída 2026-08-24 (ventana 14:42–14:47: ambas apps guardadas con 4 s de diferencia). OJO: la hipótesis inicial de que el SAVE escribía `server.js` al `.htaccess` resultó FALSA en esa caída (el `api/.htaccess` de staging nunca fue tocado) — pero la bomba sigue LATENTE: el registro de staging aún dice `"startup_file": "server.js"` y un SAVE exitoso futuro sí lo escribiría. | 1) **Ticket a hosting**: reconstruir desde root los venv (mencionar el mtime intacto/ctime del incidente para que no busquen un borrado que no ocurrió) y corregir `"startup_file"` a `index.js` en el registro del selector. 2) Mientras tanto, `heal_passenger` v4 (cron) hace **BYPASS a un node del sistema** (`/opt/alt/alt-nodejs*/root/usr/bin/node` o `/usr/local/bin/node`, ≥18) y enlaza `node_modules` a la lib del venv, que sobrevive intacta. 3) **REGLA: en esta cuenta NO usar SAVE para cambios menores** — editar la línea del `.htaccess` a mano logra lo mismo sin arriesgar el venv; SAVE solo para reconstruir el entorno de verdad, corrigiendo el campo startup a `index.js` en la misma sesión. **RESUELTO 2026-08-25 11:06 (staging 10:52):** el hosting reconstruyó el registro/vhost (ticket) y el cron reparó el resto solo — `heal_passenger` (bypass a node de sistema `/opt/alt/alt-nodejs20` + symlink + startup) y `heal-deps` (`npm ci` probe-driven; el venv tenía paquetes presentes-pero-CORRUPTOS que engañan checks de existencia — probar con `require()` real, no con `[ -d ]`). Ambos entornos quedaron sobre bypass: node de sistema + `node_modules` real, ya sin depender del venv. Cierre pendiente: retirar `diag_incidente` de `scripts/cpanel-deploy-*.sh` (publica estado interno en URL pública) y limpiar `.INVALIDO-20260824` / `~/backups-htaccess-20260824/` / markers. **`heal-deps` SE CONSERVA mientras dure el bypass**: con `node_modules` real (excluido del rsync) y el venv fuera del juego, es el ÚNICO canal de instalación de deps — un dep nuevo en `package.json` rompería la app sin él; se retira solo si se vuelve al modelo panel+venv. |
| App caída tras "recuperar" con el baile de versiones de Node (cambiar versión → SAVE → volver a 20), o tras cualquier SAVE que recree el venv: `Cannot find module 'express'` en `startup_debug.log`, o Passenger arranca un módulo vacío | Recrear el nodevenv **ROMPE el symlink `node_modules`** de la raíz de la app (`~/boveda/node_modules` → `~/nodevenv/<app>/20/lib/node_modules`); los paquetes suelen sobrevivir en la lib del venv. Variante traicionera (caída 2026-08-24): un `server.js` de **0 bytes** creado a mano "pasa" los checks de existencia pero exporta `{}` → Passenger sin app y sin error útil. | **Run NPM Install** (recrea el symlink; rápido si la lib está poblada) o esperar un tick del cron: `heal_passenger` recrea el symlink solo si la lib del venv tiene paquetes reales. Startup de 0 bytes: el heal lo trata como inexistente (`[ ! -s ]`) y fuerza `index.js`. Verificar: `/api/` → 401 JSON (nunca la página de Passenger). |
...
| **Permiso granular huérfano: la UI lo gatea pero ningún endpoint lo exige** (o al revés). Síntoma (a) el admin desmarca el permiso creyendo restringir, pero un `curl` directo al endpoint pasa igual (la restricción de la UI era cosmética); (b) un rol con el permiso granular pero sin el permiso "padre" ve la UI habilitada y recibe 403 del backend. Caso real (QA asistencia): `asistencia.periodo.crear` existía en catálogo + `permisosHierarchy` + lo gateaba la UI (`PeriodAssignModal`/`WorkerCalendarModal`), pero `POST /periodos` exigía `asistencia.guardar`. | **Front y back deben gatear el MISMO permiso por acción.** Al agregar un permiso granular al catálogo, conectar el `checkPermission()` del endpoint en el mismo commit (no confiar en que la UI "ya lo bloquea"). Fix: `POST /periodos` → `checkPermission('asistencia.periodo.crear')`. Auditar hermanos coherentes (`.ver`/`.eliminar` ya lo estaban). Grep de sanidad: por cada clave en `permisos.config.js`, `grep -r "'clave'" src/routes src/middleware` debe tener ≥1 uso. |
| **Permisos nuevos `trabajadores.solicitud.crear` / `trabajadores.solicitud.aprobar` (ficha de ingreso digital, 2026-09-07) no aparecen** en Configuración → Roles, o el usuario del rol no ve el botón "Nuevo ingreso" / la pestaña Solicitudes / el badge de Consultas, o la API responde 403 — y/o `GET /api/solicitudes-ingreso` tira `Table 'solicitudes_ingreso' doesn't exist` / `Unknown column 'fecha_nacimiento'` | La **migración 108** (`108_solicitudes_ingreso.sql`) no corrió en ese entorno, o corrió pero el rol aún no tiene las claves. La 108 hace tres cosas: 8 columnas nuevas en `trabajadores` (datos personales), tabla `solicitudes_ingreso` y el `INSERT IGNORE` en `permisos_catalogo` → `permisos_rol_v2` **solo para Super Admin (rol 1)**. El arranque del backend sincroniza el catálogo desde `permisos.config.js`, así que las claves pueden verse en el modal de Roles aunque la tabla no exista todavía — la UI "aparece" pero la API revienta. Y los permisos viven en el **JWT**: asignar la clave al rol no cambia nada hasta el siguiente login. | 1) Confirmar la migración: `https://<host>/migrate-status.txt` (auto-migrate del cron, § 3.2) debe decir `OK` con fecha posterior al deploy; si dice `FALLO`, leer `tmp/migrate-auto.log` del backend; si dice "sin pendientes" pero la tabla no existe → bootstrap-skip (§ 3.3, fix-script idempotente). 2) Configuración → Roles → asignar `solicitud.crear` a los roles de **terreno** (supervisores / prevencionistas) y `solicitud.aprobar` a los de **oficina**; ambos ya están en `permisosHierarchy.ts` (configuracion / Trabajadores). 3) **Re-login** de los usuarios afectados (regla general de permisos, fila 1452 más arriba). Recordar: el badge/contador solo se pide con `aprobar`; y una solicitud hecha sobre una obra `es_prueba=1` **no** aparece en lista ni badge sin `?incluir_prueba=true` (aislamiento § 18) — en QA staging usar una obra real o el flag. Reglas completas: `docs/reglas/rrhh-trabajadores.md` § Solicitudes de ingreso. |
| **`POST /api/trabajadores` con un RUT ya existente (activo o finiquitado)** — HISTÓRICO hasta 2026-09-07: en vez de un 409 podía **borrar EN DURO a los finiquitados** (`DELETE FROM trabajadores WHERE activo = 0`) y luego reintentar el INSERT; si el RUT duplicado era el de un **finiquitado**, el INSERT reintentado tenía éxito (201) y el registro histórico (fecha de desvinculación, ficha) desaparecía junto con TODOS los demás finiquitados sin filas dependientes; si alguno tenía asistencias/documentos, el DELETE fallaba entero y llegaba un error de FK en vez del 409 | El `create` del CRUD genérico (`crud.service.js`) tiene un **reciclaje ante `ER_DUP_ENTRY`** pensado para catálogos con `UNIQUE(nombre)` / `UNIQUE(razon_social)` (cargos, empresas…): busca la fila **inactiva homónima**, la borra y reintenta el INSERT (así "recrear" un cargo desactivado no choca con la UNIQUE). El `searchField` se elegía como `nombre` → `razon_social` → **`null`**, y con `null` la query perdía el `AND campo = ?`: para `trabajadores` (cuya UNIQUE es `rut` y no trae `nombre`) quedaba `SELECT id FROM trabajadores WHERE activo = 0 LIMIT 1` + `DELETE FROM trabajadores WHERE activo = 0` — borrado masivo de finiquitados como efecto colateral de un duplicado. Bug latente detectado en la exploración de la ficha de ingreso digital (plan 2026-09-07). | **Blindaje (2026-09-07):** el reciclaje corre **solo si hay `searchField`** (`if (searchField && ER_DUP_ENTRY)`); sin `nombre`/`razon_social` el error sigue al `errorHandler` → **409 "El registro ya existe (dato duplicado)"**, sin `SHOW COLUMNS` ni `DELETE`. El reciclaje por nombre de los catálogos sigue igual (acotado por `AND nombre = ?`). Regla: **el reciclaje es exclusivo de catálogos con UNIQUE por nombre; una tabla con otra UNIQUE (rut, patente, folio) nunca debe entrar** — si se agrega un CRUD nuevo con `UNIQUE` distinta, verificar que el body no traiga `nombre` por casualidad. Recontratar a un finiquitado = **reactivarlo** (`activo=1`, limpiar `fecha_desvinculacion`), no crearlo de nuevo; la ficha de ingreso digital bloquea el RUT de un finiquitado por lo mismo (`check-rut` mira activos e inactivos). Verificación en tests: `POST /api/trabajadores` con mock que lanza `ER_DUP_ENTRY` → 409 y **cero** queries `DELETE`; `POST /api/cargos` homónimo inactivo → sigue reciclando → 201. |
...
**Análogo horizontal (`min-w-0`):** la misma regla aplica en filas flex con texto `truncate` — cada flex item entre el contenedor `flex-1 min-w-0` y el elemento truncado necesita `min-w-0`, o el bloque desborda por encima de sus hermanos (ver fila "Botones del header global no reciben clicks" en § 6).
