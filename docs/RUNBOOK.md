...
*Última actualización: Mayo 2026 — § 4.1: caso "Reporte Semanal RRHH" (cron `0 8 * * 1`, flags `--dry`/`--to`/`--fecha`, vars `MAIL_*`/`REPORTE_TO`). § 6: filas nuevas "deploy de código acoplado a migración" (`Unknown column` post-deploy → orden deploy→migrate→verificar) y "`0` suelto en la UI" (JSX `{numero && <X/>}` → usar `!!`).*
