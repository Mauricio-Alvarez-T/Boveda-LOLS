...
| **"Web application could not be started"** en `/api/*` justo después de ENTRAR/EDITAR la app en Setup Node.js App (frontend estático sigue 200); en el panel puede aparecer `FileNotFoundError: /home/lolscl/nodevenv/...` | El formulario de cPanel guarda **Application startup file: `server.js`** (valor de la creación original que nunca importó porque el `.htaccess` real decía `index.js`). Al apretar SAVE, cPanel **reescribe el `.htaccess` desde el formulario** → Passenger intenta arrancar `server.js`, que no existe en el repo. El `FileNotFoundError` acompaña cuando el nodevenv de la app tampoco está donde el panel lo espera. | En Setup Node.js App: startup file → **`index.js`** → SAVE. Si el SAVE falla por el nodevenv: cambiar Node version a otra → SAVE → volver a 20 → SAVE (recrea el venv). Luego **Run NPM Install** → **Restart** → verificar que `/api/` responda 401 (no 500). Alternativa sin panel: restaurar el `.htaccess` a mano (playbook §7bis). Regla: **jamás guardar la app con `server.js`**; la guía de creación (§1) ya quedó corregida. Desde 2026-08-24 el cron de deploy **auto-repara** esto (`heal_passenger` en `scripts/cpanel-deploy-*.sh`): startup inexistente O de 0 bytes → `index.js`; binario de node muerto → nodevenv válido más nuevo; symlink `node_modules` roto → recreado. |
