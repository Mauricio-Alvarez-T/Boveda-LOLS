...
const dbQV = require('./src/config/db');
app.get('/api/trabajadores/:id/quick-view', authMw, async (req, res, next) => {
  try {
    const { id } = req.params;

    // 1. Worker basic info
    const [workers] = await dbQV.query(
    ...
    const [totalDocs] = await dbQV.query(`SELECT COUNT(*) as total FROM tipos_documento WHERE activo = TRUE AND obligatorio = TRUE`);
    const [completedDocs] = await dbQV.query(
    ...
    const [attendance] = await dbQV.query(
